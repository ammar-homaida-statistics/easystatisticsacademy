---
layout: default
title: "7. The Risk of Underpowered Studies"
description: "Understand why underpowered studies produce unstable results, false negatives, exaggerated estimates, and reproducibility problems."
permalink: /inference/power-sample-size/underpowered-studies/
sidebar: false
---

<section class="section section-slim">
  <div class="callout" style="background:#fff4e5; border:2px solid #ff9800; padding:2rem; border-radius:12px;">
    <h2 style="margin-top:0; color:#e65100;">🚧 Lesson Under Construction</h2>
    <p style="margin:0; color:#5d4037;">
      Version 0 frames underpower as a design flaw, not just “non-significant results”.
    </p>
  </div>
</section>

<script>
(function(){
  var KEY="esa_continue_inference_power_sample_size_lesson_v0";
  localStorage.setItem(KEY,JSON.stringify({
    url:"/inference/power-sample-size/underpowered-studies/",
    label:"Lesson 7 — The Risk of Underpowered Studies",
    ts:Date.now()
  }));
  localStorage.setItem("esa_continue_inference_last_block_v0",JSON.stringify({
    url:"/inference/power-sample-size/",
    label:"Block 5 — Power and Sample Size",
    ts:Date.now()
  }));
})();
</script>

<section class="hero hero-section">
  <div class="hero-card">
    <h1>7. The Risk of Underpowered Studies</h1>
    <p class="lead">Power is a design property. Low power causes unreliable inference even when analysis is correct.</p>
    <div class="hero-actions">
      <a class="btn btn-outline" href="/inference/power-sample-size/">Back to Block 5</a>
      <a class="btn btn-outline" href="/inference/">Statistical Inference home</a>
    </div>
  </div>
</section>

<section class="section section-slim">
  <div class="callout">
    <h2>Next step</h2>
    <div class="pill-row" style="margin-top:1rem;">
      <a class="btn" href="/inference/power-sample-size/software-and-power-analysis/">
        Next lesson: 8. Software and Practical Power Analysis →
      </a>
    </div>

    <div class="callout-side">
      <div class="mini" style="border-left:4px solid #1a73e8; padding-left:12px;">
        <div class="mini-title" style="color:#1a73e8;">Previous lesson</div>
        <div class="mini-body">
          <a href="/inference/power-sample-size/precision-vs-detectability/" style="color:#1a73e8; text-decoration:underline;">
            Lesson 6: Precision vs Detectability
          </a>
        </div>
      </div>
    </div>
  </div>
</section>